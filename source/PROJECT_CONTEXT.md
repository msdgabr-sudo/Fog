...
- use the owner-approved version `4.1.7` / code `4` and preserve all astronomical calculation and verification contracts.

## 7. Last Changes
- On 2026-08-28 the owner authorized Android version `4.1.7` / code `4`. Updated the TWA manifest, release checks, APK/AAB artifact names, and current identity documents; historical baseline records and asset-cache identifiers remain unchanged. The release identity regression gate passes, and the full suite is `88 = 77 pass + the same 11 inherited failures`. This metadata-only change does not build an AAB, deploy `main`, or alter application behavior.
...
## 12. Session Handoff
The pre-AAB branch now carries an authenticated package-scoped bridge, authoritative date-stamped 14-day native prayer plan, bundled foreground Adhan service, independent civil-time Web Adhan scheduler, local inexact Azkar voice reminders, app-private event-driven widget, and an atomic offline shell with bounded network fallback. Android identity remains `com.qiblalabs`; the owner-approved version is now `4.1.7` / code `4`, and signing material remains outside the repository. The astronomical solver, equations, store, verification cycle, protected inline runtime, and WMM runtime remain untouched. Continue with real-device/Play Console acceptance and an explicitly authorized build without changing protected astronomical behavior.
