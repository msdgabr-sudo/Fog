...
assert(activity.includes('MIN_INTERVAL_MINUTES = 10'),'Azkar native activity must enforce the Android-safe ten-minute minimum');
...
assert(activity.includes('requestExactAlarmAccessOrStart'),'Azkar native start must request exact-alarm special access when needed');
assert(activity.includes('ACTION_REQUEST_SCHEDULE_EXACT_ALARM'),'Azkar native start must use the Android exact-alarm settings contract');
assert(azScheduler.includes('MIN_INTERVAL_MINUTES = 10'),'Azkar scheduler must enforce the ten-minute minimum');
assert(azScheduler.includes('setExactAndAllowWhileIdle')&&azScheduler.includes('canScheduleExactAlarms'),'Azkar reminders must use user-granted exact idle-safe alarms');
assert(azScheduler.includes('setAndAllowWhileIdle'),'Azkar reminders must preserve an idle-safe fallback when exact access is unavailable');
assert(azScheduler.includes('catch (SecurityException ignored)'),'Azkar exact-alarm permission races must fall back without crashing');
...
assert(webSync.includes("maybeNativeSync('startup')"),'previously activated native prayer schedules or widgets must refresh when the app is opened again');
assert(webSync.includes("maybeNativeSync('location-changed')"),'an activated native prayer schedule or widget must refresh after trusted location changes');
assert(webSync.includes("maybeNativeSync('runtime-ready')"),'an activated native prayer schedule or widget must refresh only after the authoritative prayer runtime reports a valid schedule');
...
assert(azWeb.includes('Math.max(10,n)'),'Azkar Android bridge must enforce the ten-minute native minimum');
assert(azWeb.includes('enforceNativeIntervals'),'Azkar Android bridge must hide unsupported sub-ten-minute choices');
...
assert(azHost.includes('Math.max(10,Math.min(1440,n))'),'top-level Azkar host must clamp native reminders to at least ten minutes');
...
console.log('Azkar native toggle + ten-minute exact background scheduling with idle-safe fallback: PASS');
