...
// Legacy Code 3 PrayerWidgetSyncActivity accepts qiblaastro://prayer-sync,
// token, notify, plan, t_*, m_*, advance and profile. Extra widgetOnly is ignored by
// Code 3 and understood by Code 5; therefore notify MUST carry the real master state.
...
assert(prayer.includes("q.set('widgetOnly','1')"),'Code 5 widget-only isolation marker missing');
assert(!prayer.includes("q.set('notify',widgetOnly?'0':"),'legacy Code 3 must never receive a synthetic notify=0 during widget refresh');
assert(code5Native.includes('widgetOnly="1".equals')&&code5Native.includes('if(widgetOnly){finish();return;}'),'Code 5 must keep widget-only refresh isolated from permission UI');
assert(code5Native.includes('if(!widgetOnly)e.putString("mode_"+id'),'Code 5 widget refresh must preserve Adhan delivery choices');
...
console.log('Identity/DAL/token: stable; Prayer/Adhan: backward-compatible; Azkar: backward-compatible; delegated location: preserved.');
