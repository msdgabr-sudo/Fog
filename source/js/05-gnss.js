...
function updateQiblaFromPosition(){
  ...
  QT=calcQibla(LAT,LON);
  if(!Number.isFinite(QT)){
    showGnssUnavailable('إحداثيات الموقع غير صالحة — أعد محاولة تحديد الموقع');
    return;
  }
  var magneticReady=false;
  try{magneticReady=refreshMdeclFromTrustedGnss(new Date());}
  catch(_){
    MDECL_READY=false;
    MDECL_STATUS='unavailable';
    MDECL_FIELD=null;
  }
  QM=magneticReady?((QT-MDECL)+360)%360:QT;
  ...
  set('mag-d',magneticReady?(MDECL>=0?'+':'')+MDECL.toFixed(2)+'°':MDECL_STATUS==='blackout'?'⚠ مجال مغناطيسي ضعيف':'---');
  set('q-mag',magneticReady?QM.toFixed(2)+'°':'---');
  set('cfg-qm',magneticReady?QM.toFixed(2)+'°':'---');
  set('cfg-md',magneticReady?(MDECL>=0?'+':'')+MDECL.toFixed(2)+'°':'---');
  set('mag-decl-inline',magneticReady?MDECL.toFixed(2):'---');
...
function tryBrowserGPS(){
  ...
  try{
    ...
    navigator.geolocation.getCurrentPosition(
      function(pos){
        try{
          var lat=pos&&pos.coords?Number(pos.coords.latitude):Number.NaN;
          var lon=pos&&pos.coords?Number(pos.coords.longitude):Number.NaN;
          ...
          try{
            window._gnssWatchId = navigator.geolocation.watchPosition(
              function(p2){
                try{
                  var nextLat=p2&&p2.coords?Number(p2.coords.latitude):Number.NaN;
                  var nextLon=p2&&p2.coords?Number(p2.coords.longitude):Number.NaN;
                  if(Number.isFinite(nextLat)&&nextLat >= -90&&nextLat <= 90&&Number.isFinite(nextLon)&&nextLon >= -180&&nextLon <= 180&&p2.coords.accuracy<(gnssAccuracy||9999)){
                    LAT=nextLat;
                    LON=nextLon;
