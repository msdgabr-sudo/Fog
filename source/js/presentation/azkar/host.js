...
(function(root){
  ...
  function tokenFromHash(){try{var raw=String(root.location.hash||'').replace(/^#/,'');var params=new URLSearchParams(raw),token=params.get('nativeToken')||'';if(token)return token;try{var decoded=decodeURIComponent(raw);if(decoded!==raw)return new URLSearchParams(decoded).get('nativeToken')||'';}catch(_){}return '';}catch(_){return '';}}
